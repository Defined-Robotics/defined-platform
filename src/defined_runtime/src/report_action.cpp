/*!
 * \file report_action.cpp
 * \brief Implementation of ReportAction — publishes a message and logs it.
 */
...
ReportAction::ReportAction(const std::string& name, const BT::NodeConfig& config,
...
BT::PortsList ReportAction::providedPorts() {
  return {
      ...
      BT::InputPort<std::string>("topic", "/task_reports", "ROS topic to publish to"),
      BT::InputPort<std::string>("level", "info", "Severity: debug, info, warn, error"),
